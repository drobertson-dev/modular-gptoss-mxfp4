---
title: "1D Convolution Op (PyTorch)"
description: "In this puzzle, we'll take the exact same 1D convolution kernel from Puzzle 17 and integrate it with PyTorch using the CustomOpLibrary instead of MAX Graph."
---

# 1D Convolution Op (PyTorch)
...
In this puzzle, we'll take the exact same 1D convolution kernel from Puzzle 17 and integrate it with PyTorch using the [CustomOpLibrary](https://docs.modular.com/max/api/python/torch/CustomOpLibrary/) instead of MAX Graph.
...
```python
# X This won't work - no output tensor
result = torch.compile(conv1d)(input_tensor, kernel_tensor)

# OK This works - pre-allocated output tensor
output_tensor = torch.empty_like(input_tensor)
torch.compile(conv1d)(output_tensor, input_tensor, kernel_tensor)
```
