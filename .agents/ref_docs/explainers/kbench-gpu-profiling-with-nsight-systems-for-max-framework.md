---
title: "GPU Profiling with NSight Systems for MAX Framework"
description: "A guide to using NVIDIA Nsight Systems to analyze performance of MAX framework applications on GPUs."
---

# GPU Profiling with NSight Systems for MAX Framework

A guide to using NVIDIA Nsight Systems to analyze performance of MAX framework applications on GPUs.
...
You should use `nsys` when you need to understand where time is being spent across your application, such as which kernels are slow. You can then deep-dive into specific kernel performance with Nsight Compute, or our purpose-built `kbench` tool that helps you benchmark, autotune, and analyze Mojo kernel performance.

![](https://docs.modular.com/assets/images/nsight-systems-0a1aae9b8ad38c67d454ec47770d3e9f.png)

## Requirements

- [NVIDIA GPU compatible with MAX](https://docs.modular.com/max/packages#gpu-compatibility)
- [NVIDIA driver version compatible with MAX](https://docs.modular.com/max/packages#gpu-software-requirements)
...
Otherwise, you can download it from [NVIDIA's Nsight Systems page](https://developer.nvidia.com/nsight-systems/get-started) or install it with your package manager. For example, on Ubuntu/Debian:
...
For details about the `nsys` command options, see the [Nsight Systems User Guide](https://docs.nvidia.com/nsight-systems/UserGuide/index.html).
...
1. Make sure you're in a project environment with the `modular` package installed.

1. The next step deploys Google Gemma 3, so in order for MAX to download the model weights, you must first agree to the [Gemma 3 license](https://huggingface.co/google/gemma-3-12b-it) and set your HF Access Token:

```bash
export HF_TOKEN="hf_..."
```

3. Start the MAX server (`max serve`) with `nsys launch` (this doesn't start profiling yet):
...
4. Wait until the server is ready-you'll see a message like this in your terminal:
...
6. In the second terminal, use `max benchmark` to run a benchmark workload:
...
1. Install NVIDIA Nsight Systems on your local system (available for Windows, macOS, and Linux).

1. Copy the `profile.nsys-rep` file to your local machine (such as via `scp`).

1. Double-click the `.nsys-rep` file to open it in Nsight Systems. You should see a timeline showing the profile results, similar to the example in figure 1.
...
For detailed instructions for analyzing the profile results, see the [Nsight Systems Post-Collection Analysis Guide](https://docs.nvidia.com/nsight-systems/AnalysisGuide/index.html).
