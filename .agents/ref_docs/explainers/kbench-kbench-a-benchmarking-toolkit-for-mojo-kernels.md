---
title: "KBench: A Benchmarking Toolkit for Mojo Kernels"
description: "A Python-based toolkit for building and executing Mojo kernel benchmarks across parameter combinations for autotuning and performance analysis."
---

# KBench: A Benchmarking Toolkit for Mojo Kernels

A Python-based toolkit for building and executing Mojo kernel benchmarks across parameter combinations for autotuning and performance analysis.
...
See `sample.mojo` for a complete example template.
...
See `test.yaml` for an example template.
...
![ toolkit](data/kbench_toolkit.png)
...
```python
instance_list = product(params, values)
for all specs in yaml
```
...
The first spec expands into 4 instances (2 shapes x 2 stages). The second spec
...
tuning parameters `T`, you might want `expansion(S) x expansion(T)` rather than
`expansion(S x T)`. This writes results for each shape to a separate output
file.
...
1. Create a YAML config file with a `.py` file in the `file` path.

1. Create a Python script. In your Python script, import the required functions from
   `bencher_utils`:
