---
title: "Data Types and Memory Representation in MAX Tensors"
description: "A guide to using the DType enum in MAX for tensor precision, memory optimization, and framework interoperability."
---

# Data Types and Memory Representation in MAX Tensors

A guide to using the DType enum in MAX for tensor precision, memory optimization, and framework interoperability.

Data types (dtypes) define how numbers are stored in tensors. A _dtype_ specifies how each element in a tensor is
represented in memory, and every tensor has exactly one dtype that applies to all its elements. Choosing the right dtype
affects your model's memory usage, numerical precision, and compatibility with different hardware.
...
In this example, the `ones()` function creates a tensor filled with ones, and the `constant()` function creates a tensor
filled with the given values. The `dtype` parameter is used to specify the dtype of the tensor.
...
The `cast()` method is used to convert a tensor from one dtype to another. This is useful when you need to convert a
tensor from a floating-point type to an integer type, or from a higher precision type to a lower precision type. For
example:
...
In this example, the original tensor is a `float32` tensor, after casting to `int32`, the tensor is a `int32` tensor,
and after casting to `float64`, the tensor is a `float64` tensor.
...
DLPack is a standardized in-memory tensor format and protocol that lets array and tensor libraries share data across
devices and frameworks with zero or minimal copies.

The recommended way to convert NumPy arrays to MAX tensors is through DLPack, which enables zero-copy conversion when
possible:
...
In this example, the `from_dlpack()` method converts the NumPy array to a MAX tensor. You can use this method when
converting data from other libraries to MAX.

**NumPy compatibility**

Not all MAX dtypes have NumPy equivalents. For example, `bfloat16` and `float8` types are not natively supported by
NumPy. When working with these types, you may need to cast to a compatible type first.

MAX also provides dtype conversion for PyTorch and NumPy integration. The `from_torch()` method converts a PyTorch dtype
to a MAX dtype. For example:
...
Understanding dtype memory usage is critical for deploying large models. The `size_in_bytes` property lets you calculate
