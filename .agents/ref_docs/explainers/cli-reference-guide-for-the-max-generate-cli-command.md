---
title: "Reference Guide for the MAX generate CLI Command"
description: "Detailed documentation of the max generate command for testing and debugging model outputs."
---

# Reference Guide for the MAX generate CLI Command

Detailed documentation of the max generate command for testing and debugging model outputs.
...
You can adjust parameters like `--max-batch-size` and `--max-length` depending on your system's available resources such
as GPU memory.
...
The frequency penalty to apply to the model's output. A positive value will penalize new tokens based on their frequency
...
Images to include along with prompt, specified as URLs. The images are ignored if the model does not support image
inputs.
...
Float that represents the minimum probability for a token to be considered, relative to the probability of the most
likely token. Must be in [0, 1]. Set to 0 to disable this.
...
The presence penalty to apply to the model's output. A positive value will penalize new tokens that have already
appeared in the generated text at least once.
...
The repetition penalty to apply to the model's output. Values > 1 will penalize new tokens that have already appeared in
the generated text at least once.
...
A list of detokenized sequences that can be used as stop criteria when generating a new sequence. Can be specified
multiple times.
