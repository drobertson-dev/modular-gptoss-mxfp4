...
description: "Welcome to the final frontier of GPU matrix multiplication optimization! In this puzzle, we'll explore Tensor Cores - specialized hardware units designed to accelerate mixed-precision matrix operations at unprecedented speeds."
---
...
- **Fixed tile sizes**: Operations work on specific matrix fragment sizes (e.g., 1688 for FP32)
...
a_reg = mma_op.load_a(A_mma_tile)           # Load 16x8 fragment
b_reg = mma_op.load_b(B_mma_tile)           # Load 8x8 fragment
c_reg = mma_op.load_c(C_mma_tile)           # Load 16x8 accumulator
d_reg = mma_op.mma_op(a_reg, b_reg, c_reg)  # D = AxB + C
...
# - mma_op(): Perform D = AxB + C operation
...
- **float32**: 1688 or 1684
- **half-precision**: 16816
- **float8**: 16832

**AMD GPUs:**

- **float32**: 16164
- **half-precision**: 161616 or 32328

**This puzzle uses FP32 with 1688 fragments:**
...
```txt
A fragment (16x8)  x  B fragment (8x8)  +  C fragment (16x8)  =  D fragment (16x8)

    16 rows             8 rows               16 rows              16 rows
    8 cols              8 cols               8 cols               8 cols
      |                   |                    |                    |
   [A data]         x   [B data]         +   [C data]         =  [D result]
```

This means each Tensor Core instruction computes a 168 output tile by multiplying a 168 tile from A with an 88 tile from B, then adding it to the existing 168 accumulator.
...
# Each warp handles a WMxWN tile of the output
C_warp_tile = C_block_tile.tileWM, WN
```

**Warp organization example**(with BM=128, BN=64, WM=32, WN=32):

```txt
Block (128x64) contains 8 warps arranged as:
...
Total: 4x2 = 8 warps, each handling 32x32 output region
```
...
Example goal:  complete the `tensor_core_matrix_multiplication` function. The skeleton builds on the tiled approach but uses actual Tensor Core hardware operations.
...
:
    ...
    out_tile = output.tileTILE_SIZE, TILE_SIZE, Int(block_idx.x)
    )
    ...
    for idx in range(size // TILE_SIZE):  # Iterate over K tiles
        ...
        a_tile = a.tileTILE_SIZE, TILE_SIZE, idx)
        b_tile = b.tileTILE_SIZE, TILE_SIZE)
...
- **From:**Individual element results  **To:**168 matrix fragment results
...
- **Warp tiling**: `WM=32, WN=32` (each warp handles a 3232 output region)
- **MMA fragments**: `1688` (hardware-defined matrix fragment sizes)
- **Warps per block**: 8 warps (organized as 42 in the BMBN block)

**Why these specific sizes?**

- **BM=128, BN=64**: Larger than tiled version (3232) to better utilize Tensor Cores
- **WM=WN=32**: Multiple of WARP_SIZE and contains 24=8 MMA fragments (3216=2, 328=4)
- **MMA 1688**: Fixed by hardware - this is what the Tensor Cores physically compute
- **8 warps**: BMWM  BNWN = 12832  6432 = 42 = 8 warps per block

**How warps map to MMA fragments:**

```txt
Each 32x32 warp tile contains multiple 16x8 MMA fragments:
...
2 fragments across (32/16=2) x 4 fragments down (32/8=4) = 8 MMA operations per warp per K-tile
```
...
OK TENSOR CORE ACCURACY TEST PASSED!
--- Test 2: Idiomatic Tiled vs CPU Reference ---
OK IDIOMATIC TILED ACCURACY TEST PASSED!
ALL TESTS PASSED!
```
...
:
    ...
    C_block_tile = C.tileBM, BN, Int(block_idx.x))
    C_warp_tile = C_block_tile.tileWM, WN
    ...
    for k_i in range(K // BK):
        ...
        A_dram_tile = A.tileBM, BK, k_i)
        B_dram_tile = B.tileBK, BN)

        copy_dram_to_sram_async
            thread_layout = Layout.row_major(4, 8),
            num_threads=256,
            block_dim_count=BLOCK_DIM_COUNT,
        , A_dram_tile.vectorize[1, 4]())
        copy_dram_to_sram_async
            thread_layout = Layout.row_major(4, 8),
            num_threads=256,
            block_dim_count=BLOCK_DIM_COUNT,
        , B_dram_tile.vectorize[1, 4]())

        async_copy_wait_all()
        barrier()

        if warp_is_active:
            A_warp_tile = A_sram_tile.tileWM, BK
            B_warp_tile = B_sram_tile.tileBK, WN
            ...
            for mma_k in range(BK // MMA_K):
                ...
                for mma_m in range(WM // MMA_M):
                    ...
                    for mma_n in range(WN // MMA_N):
                        A_mma_tile = A_warp_tile.tileMMA_M, MMA_K
                        B_mma_tile = B_warp_tile.tileMMA_K, MMA_N
                        C_mma_tile = C_warp_accum.tileMMA_M, MMA_N
    ...
    if warp_is_active:
        ...
        for mma_m in range(WM // MMA_M):
            ...
            @parameter
            for mma_n in range(WN // MMA_N):
                var C_mma_tile = C_warp_tile.tileMMA_M, MMA_N
                Acc_mma_tile = C_warp_accum.tileMMA_M, MMA_N
...
1. **Warp organization**
   ...
   - Maps warps to output tiles: each warp handles a `WMWN` region
...
3. **Tensor Core operations**
   - `load_a(A_mma_tile)`: Loads 168 matrix A fragment into registers
   - `load_b(B_mma_tile)`: Loads 88 matrix B fragment into registers
   - `load_c(C_mma_tile)`: Loads 168 accumulator fragment
   - `mma_op(a_reg, b_reg, c_reg)`: Computes D = AB + C using specialized hardware
   - `store_d(C_mma_tile, d_reg)`: Stores 168 result fragment
