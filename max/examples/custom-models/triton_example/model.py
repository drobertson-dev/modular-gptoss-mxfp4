...
class RotaryEmbedding(torch.nn.Module):
    ...
    def forward(
        ...
        idx = (
            torch.arange(num_tokens, device=query.device, dtype=torch.long)
            + offset
        )
...
class Cache:
    ...
    def extend(self, k, v):
        ...
        indices = (
            torch.arange(0, n_ctx, device=k.device, dtype=torch.long)
            + self.offset
        )
...
class Transformer(torch.nn.Module):
    ...
    def from_checkpoint(
        ...
        for name, param in model.named_parameters():
            ...
            if "mlp1" in name:
                if "weight" in name:
                    loaded_tensor, scales = quantize_mx4(
                        loaded_tensor.mT.contiguous()
                    )
    ...
    def __init__(self, checkpoint: str, context: int, device: torch.device):
        ...
        with torch.cuda.graph(self.graph):
            self.logits = self.model(
                self.input_token[None, :], caches=self.caches
            )[0]
