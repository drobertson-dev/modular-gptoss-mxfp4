...
def _attn_fwd(
    ...
    # load scales
    qk_scale = sm_scale
    q = Q.load([off_z, off_h, start_m * BLOCK_M, 0]).reshape(
        [BLOCK_M, HEAD_DIM]
    )
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, sinks, sm_scale, bandwidth, start_q):
        ...
        n_pad_size = (
            BLOCK_N - n_kv_ctx % BLOCK_N if n_kv_ctx % BLOCK_N != 0 else 0
        )
...
def attention_ref(
    ...
    sinks = sinks.view(
        1, num_key_value_heads, num_key_value_groups, 1, 1
    ).float()
    key = key.unsqueeze(3)
    ...
    if sliding_window:
        too_old = pos_keys[None, :] < (
            pos_queries[:, None] - sliding_window + 1
        )
        mask.masked_fill_(too_old, float("-inf"))

    logits = (
        torch.einsum("bqhmd,bkhmd->bhmqk", query.float(), key.float())
        * sm_scale
    )
...
def test_eq(
    ...
    sinks = (
        torch.randn(num_key_value_heads * num_key_value_groups)
        .bfloat16()
        .cuda()
    )
