...
fn _bf16_scale_e8m0_bits(
    ...
    if exp == UInt16(0):
        ...
        if kp >= UInt16(7):
            ...
            var mant_new = (mant - (UInt16(1) << p)) << (UInt16(7) - p)
    ...
    if scale_exp == UInt8(0):
        ...
        if (mant_full & UInt16(1)) != UInt16(0) and (
            mant_new & UInt16(1)
        ) != UInt16(0):
            mant_new += UInt16(1)
        return sign | (mant_new & UInt16(0x7F))
...
fn decode_mxfp4_packbits_u32_to_8xbf16_scaled_e8m0(
    ...
    # Keep behavior bit-identical with the established decode path used by
    # grouped/MoE kernels while still accepting an exponent byte at callsites.
